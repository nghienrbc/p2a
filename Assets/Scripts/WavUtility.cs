...
public static class WavUtility
{
    ...
    {
        // Giải mã WAV thành AudioClip
        int sampleCount = data.Length / 2; // Dựa trên sample 16-bit stereo hoặc mono
        float[] samples = new float[sampleCount];

        // Chuyển đổi dữ liệu byte thành mẫu âm thanh
        for (int i = 0; i < sampleCount; i++)
        {
            samples[i] = (short)(data[i * 2] | (data[i * 2 + 1] << 8)) / 32768.0f; // Chuyển đổi 16-bit PCM
        }
