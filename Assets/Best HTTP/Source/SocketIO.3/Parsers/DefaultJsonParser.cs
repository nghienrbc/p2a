...
namespace BestHTTP.SocketIO3.Parsers
{
    ...
    public sealed class DefaultJsonParser : IParser
    {
        ...
        public IncomingPacket Parse(SocketManager manager, string from)
        {
            ...
            _ReadData _readData = new _ReadData();
            ...
            switch (socketIOEvent)
            {
                ...
                case SocketIOEventTypes.Connect:
                    ...
                    if (!string.IsNullOrEmpty(payload))
                        _readData = ReadData(manager, packet, payload);
                    break;
            ...
            packet.EventName = _readData.eventName;
            
            if (_readData.args != null)
            {
                if (_readData.args.Length == 1)
                    packet.DecodedArg = _readData.args[0];
                else
                    packet.DecodedArgs = _readData.args;
            }
        ...
        public IncomingPacket MergeAttachements(SocketManager manager, IncomingPacket packet)
        {
            ...
            _ReadData readData = ReadData(manager, packet, payload);

            packet.EventName = readData.eventName;
            
            if (readData.args != null)
            {
                if (readData.args.Length == 1)
                    packet.DecodedArg = readData.args[0];
                else
                    packet.DecodedArgs = readData.args;
            }

            return packet;
        }

        private _ReadData ReadData(SocketManager manager, IncomingPacket packet, string payload)
        {
